    ...
    pub fn encode_pcm16_to_wav(
        ...
        let mut cursor = Cursor::new(Vec::new());
        {
            let mut writer = hound::WavWriter::new(&mut cursor, spec)
                .map_err(|e| VoxForgError::AudioProcessing(format!("Failed to init WAV writer: {}", e)))?;

            for &sample in pcm_samples {
                writer.write_sample(sample)
                    .map_err(|e| VoxForgError::AudioProcessing(format!("Failed to write sample: {}", e)))?;
            }

            writer.finalize()
                .map_err(|e| VoxForgError::AudioProcessing(format!("Failed to finalize WAV: {}", e)))?;
        }
